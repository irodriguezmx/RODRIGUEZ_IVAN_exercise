...
public class MembershipsServiceImpl implements MembershipsService {
    ...
    public Membership assignRoleToMembership(@NonNull Membership membership) {
        ...
        UUID teamId = ofNullable(membership).map(Membership::getTeamId)
                .orElseThrow(() -> new InvalidArgumentException(Role.class));
        UUID userId = ofNullable(membership).map(Membership::getUserId)
                .orElseThrow(() -> new InvalidArgumentException(Role.class));

        roleRepository.findById(roleId).orElseThrow(() -> new ResourceNotFoundException(Role.class, roleId));

        if (teamsService == null || teamsService.getTeam(teamId) == null) {
